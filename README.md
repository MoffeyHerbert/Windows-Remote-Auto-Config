...
### 2. LGPO.exe
LGPO.exe是命令行公用程式，其设计目的是要协助自动化本机群组原则的管理。 

使用本机原则可提供系统管理员一个简单的方式来验证群组原则设定的效果，同时也很适合用于管理未加入网域的系统。

LGPO.exe可以从登录原则汇入和套用设定(Registry.pol) 档案、安全性范本、进阶稽核备份档，以及格式化的「LGPO 文字」档案。

它可以汇出本机原则至GPO 备份，将登录原则档案的内容汇出为可编辑的「LGPO 文字」格式，而且可以从LGPO文字档建置登录原则档案。

您可以在Microsoft 安全性指引部落格或下载工具，找到LGPO工具的档。

要查找有关 LGPO 的更多信息，请参阅微软
[Microsoft 安全性合规性工具组- 如何使用](https://learn.microsoft.com/zh-tw/windows/security/operating-system-security/device-management/windows-security-configuration-framework/security-compliance-toolkit-10)。


## 使用方法
1. 确保 `ntrights.exe` 工具在脚本运行的路径中，或者修改脚本以指向正确的路径。
2. 确保 `lgpo.exe` 工具在脚本运行的路径中，或者修改脚本以指向正确的路径。
